...
def get_random(length):
    arr = [i for i in range(10**(length-1), 10**length-1)]
...
def init(length: int = 6, heart: int = 6, post_init=False):
    ...
    st.session_state.word = get_random(length)
...
def main():
    ...
    reset.button(f'New Numble ({st.session_state.length})', on_click=restart)
    ...
    with settings.expander('Settings'):
        ...
        st.slider('Set length of the number', 3, 16, 6, key='length', on_change=restart)

    placeholder, debug = st.empty(), st.empty()
    print(f"Number to guess is {st.session_state.word}")
    guess = placeholder.text_input('Guess the numble', key=st.session_state.input, max_chars=st.session_state.length).lower()

    if not guess or contains_non_digit(guess) or len(guess) != st.session_state.length:
        # don't show warning at start of game
        if st.session_state.lives < st.session_state.heart:
            debug.warning('Please input valid number')
    ...
    if st.session_state.lives == 0:
        debug.error(f"**You lost**, the number was **{st.session_state.word}** 😓")
        placeholder.empty()
    elif guess == st.session_state.word:
        debug.success(f"**You win**, the number was {st.session_state.word} 🎈")
